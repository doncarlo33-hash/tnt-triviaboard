...
import { numberOrZero, normalizeIdList, normalizeWagerMap, clampFinalWagers, getTeamTotal } from './helpers.js';
...
export function resetLoadedGameState(current) {
  ...
  draft.finalRound = {
    ...draft.finalRound,
    questions: draft.finalRound.questions.map((question) => ({
      ...question,
      wagers: {},
      correctTeamIds: [],
    })),
...
export function normalizeQuestions(candidateQuestions) {
  ...
  const normalized = STANDARD_POINT_VALUES.map((points, questionIndex) => {
    ...
    const id = sourceQuestion?.id && !usedIds.has(sourceQuestion.id) ? sourceQuestion.id : crypto.randomUUID();
    usedIds.add(id);
  ...
  const boosterId = boosterQuestion?.id && !usedIds.has(boosterQuestion.id) ? boosterQuestion.id : crypto.randomUUID();
...
export function normalizeAwardMoment(awardMoment, teams) {
  ...
  return {
    id: String(awardMoment.id || crypto.randomUUID()),
