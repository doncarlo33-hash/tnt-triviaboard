      ...
      if (theme === 'theme-cyberpunk') {
        ...
        particles.forEach(p => {
          ...
          if (theme === 'theme-classic') {
            ...
            if (!p.fireworkInit) {
               p.fireworkInit = true;
               p.burstGroup = Math.floor(Math.random() * 6); // 6 simultaneous bursts
               p.angle = Math.random() * Math.PI * 2;
               p.speed = Math.random() * 6 + 2;
            }
            
            const burstDuration = 2.5; // Seconds
            const cycleTime = (time + p.burstGroup * 0.5) % burstDuration;
            
            // Explode when a new cycle starts
            if (cycleTime < 0.02) {
               const cycleIndex = Math.floor((time + p.burstGroup * 0.5) / burstDuration);
