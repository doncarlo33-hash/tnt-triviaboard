...
import { createDefaultState, normalizeState, loadState, getCurrentCategoryIndex, getActiveQuestion, getRankedTeams } from './utils/state.js';
...
export default function App() {
  ...
  const isPlayerScreen = screen === "player";
  ...
  const handleIncomingMessage = useCallback((msg) => {
    if (msg && msg.type === 'CLAIM_TEAM' && msg.teamId && msg.playerId) {
      updateState(draft => {
        if (!draft.teamClaims) draft.teamClaims = {};
        for (const [tId, pId] of Object.entries(draft.teamClaims)) {
          if (pId === msg.playerId) delete draft.teamClaims[tId];
        }
        draft.teamClaims[msg.teamId] = msg.playerId;
      });
    }
    else if (msg && msg.type === 'RELEASE_TEAM' && msg.teamId && msg.playerId) {
      updateState(draft => {
        if (draft.teamClaims && draft.teamClaims[msg.teamId] === msg.playerId) {
          delete draft.teamClaims[msg.teamId];
        }
      });
    }
      ...
      updateState(draft => {
        ...
        draft.submittedAnswers[msg.teamId] = msg.answer;
      });
    }
  }, [updateState]);
  ...
  function openQuestionModal(categoryIndex, questionIndex) {
    ...
    setState((current) => {
      ...
      draft.displayView = "board";
      draft.submittedAnswers = {}; // Reset submitted answers for the new question
      return draft;
    });
  }
  ...
  function closeQuestionModal() {
    ...
    setState((current) => {
      ...
      if (question) {
        ...
        // When a question is officially closed, anyone who didn't get points missed it and loses their fire
        const awarded = Array.isArray(question.awardedTeamIds) ? question.awardedTeamIds : [];
        draft.teams.forEach(team => {
          if (!awarded.includes(team.id)) {
            team.onFire = false;
          }
        });
      }
  ...
  function applyBoardScore(teamIds, categoryIndex, question) {
    updateState((draft) => {
      ...
      // Update On Fire status for all teams
      draft.teams.forEach(team => {
        // Calculate the total number of categories this team has completely swept
        const sweptCategoriesCount = draft.categories.filter(category => 
          category.questions && 
          category.questions.length > 0 && 
          category.questions.every(q => Array.isArray(q.awardedTeamIds) && q.awardedTeamIds.includes(team.id))
        ).length;

        if (!nextTeamIds.includes(team.id)) {
          // If the team missed this question (did not get points), they lose their fire status
          team.onFire = false;
        } else {
          // If they got it right, check if they swept a brand new category
          if (sweptCategoriesCount > (team.sweptCount || 0)) {
            team.onFire = true;
          }
        }
        
        // Always update their swept count so we know when they sweep a new one
        team.sweptCount = sweptCategoriesCount;
      });
